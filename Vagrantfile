...
# Ensure we do not start any VM automatically.
define_opts = {
  autostart: false
}.freeze
...
Vagrant.configure(2) do |config|
  config.vm.provider :digital_ocean do |provider, override|
    # provider.image  = 'ubuntu-16-04-x64' # vagrant digitalocean-list images $DIGITAL_OCEAN_TOKEN
    provider.region = 'nyc3'             # vagrant digitalocean-list regions $DIGITAL_OCEAN_TOKEN
    provider.size   = 's-2vcpu-4gb'      # vagrant digitalocean-list sizes $DIGITAL_OCEAN_TOKEN
  ...
  PRJCT_DIR = ENV['VAGRANT_PROJECT_DIR'] || Dir.pwd
  config.vm.synced_folder PRJCT_DIR, '/julia-install',  type: 'rsync',
                                                        create: true,
                                                        owner: 'root',
                                                        rsync__exclude: '.git/',
                                                        rsync__auto: true
  'ubuntu-1604-do'.tap do |box|
    config.vm.define box, define_opts do |config|
      # Running in the background switch these
      config.trigger.after(:up) { |t| t.run = {inline: "bash -c 'vagrant rsync-auto #{box} &'"} }
      config.vm.provider(:digital_ocean) { |p| p.image = 'ubuntu-16-04-x64' }
...
def linux_common(config,
  ...
  config.vm.provision 'clean all artifacts from tmp', type: 'shell', inline: <<-SHELL
    rm -rf /tmp/*
  SHELL

  # Test build and install of the latest minor update
  config.vm.provision 'Julia install', type: 'shell', inline: <<-SHELL
    cd /julia-install
      bin/julia-install julia 1.0
  SHELL
